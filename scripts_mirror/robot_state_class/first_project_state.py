...
class Search_Ball(smach.State):
    ...
    def execute(self, ud):
        ...
        if self.preempt_requested():
            self.service_preempt()
            return 'failed'
        (ud.ball_x,ud.ball_y,ud.ball_theta,has_ball) = findball.findBall().find_ball_turn(math.pi)
        if has_ball == True:
            return 'successed'
        else:
            return 'failed'
...
class Search_Ball_CW(smach.State):
    ...
    def execute(self, ud):
        ...
        if self.preempt_requested():
            self.service_preempt()
            return 'failed'
        (ud.ball_x,ud.ball_y,ud.ball_theta) = findball.findBall().find_ball_turn_cw(math.pi/1.5)
        return 'successed'
