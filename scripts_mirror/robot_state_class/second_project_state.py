...
class Shoot_Adjust_2(smach.State):
    ...
    def execute(self, ud):
        ...
        rospy.loginfo("x = %s"%ud.column_x)
        rospy.loginfo("theta = %s"%ud.column_theta)
        self.move_cmd.move_to(x = ud.column_x - 2.30)
        self.move_cmd.move_to( yaw=ud.column_theta)
...
class Move_Point_To_Shoot(smach.State):
    ...
    def execute(self, ud):
        ...
        (current_x,current_y) = self.cmd_position.get_robot_current_x_y()
        self.move_cmd.move_to(x = 7.45 - current_x,y = 8.5 - current_y)
        self.move_cmd.move_to(yaw = math.pi / 5)
        return 'successed'
...
class Move_To_Find_Ball(smach.State):
    ...
    def execute(self, ud):
        if self.preempt_requested():
            ...
            return 'failed'
        (current_x,current_y) = self.cmd_position.get_robot_current_x_y()
        self.move_cmd.move_to(x = 5.3 - current_x,y = -1.3 - current_y)
...
class Adjust_To_Shoot(smach.State):
    ...
    def execute(self, ud):
        if self.preempt_requested():
            ...
            return 'failed'
        (current_x,current_y) = self.cmd_position.get_robot_current_x_y()
        self.move_cmd.move_to(x = 7.4 - current_x,y = 8.8 - current_y)
...
class Shoot_Adjust_Second(smach.State):
    ...
    def execute(self, ud):
        ...
        (current_x,current_y,current_w) = self.cmd_position.get_robot_current_x_y_w()
        self.move_cmd.move_to(x = 7.4 - current_x,y = 8.8 - current_y,yaw= math.pi/7 -current_w)
        self.turn_cmd.turn_to(-math.pi/5)
        return "successed"
        # self.turn_cmd.turn_to(-math.pi / 8)
...
class Move_To_Three_Point_Line(smach.State):
    ...
    def execute(self, ud):
        ...
        (current_x,current_y,current_w) = self.cmd_position.get_robot_current_x_y_w()
        #定位柱的位置 8, -10.925
        angular = math.atan2( 10.925- (3.9),8 - 4.05)
        self.move_cmd.move_to(x = 1.45,y = 3.9,yaw = angular - current_w)
        (current_x,current_y,current_w) = self.cmd_position.get_robot_current_x_y_w()
